...
void GPIO_Configure(void)
{
	...
	/******************system configure*******************/
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_6;//LED
...
// 	GPIO_Init(GPIOA, &GPIO_InitStructure);
	...
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;              //   输出极性   起始电平为低电平
...
// 	mMaininf.mUart5.mReceiveFlag = 0;      //    没有接收导数据
	...
	for(i = 0;i<5;i++)
	{
		mMaininf.mWork.mWorkLedStatus[i] = 0;
	}
	//    工作相关
	...
	mMaininf.mWork.mWorkDropDataFar = 0;
	mMaininf.mWork.mWorkDropDataNear = 0;
}
