...
void api_UART2_Display_SendCMDData(u8 mode,u8 cmd,u8 status,u16 dat1,u16 dat2)
{
	...
	mMaininf.mUart2.SendBuf[5] = dat1 >> 8;
	mMaininf.mUart2.SendBuf[6] = dat1 & 0xff;
	mMaininf.mUart2.SendBuf[7] = dat2 >> 8;
	mMaininf.mUart2.SendBuf[8] = dat2 & 0xff;;
	mMaininf.mUart2.SendBuf[9] = mMaininf.mUart2.SendBuf[2] ^ mMaininf.mUart2.SendBuf[3] ^
								 mMaininf.mUart2.SendBuf[4] ^ mMaininf.mUart2.SendBuf[5] ^
								 mMaininf.mUart2.SendBuf[6] ^ mMaininf.mUart2.SendBuf[7] ^
								 mMaininf.mUart2.SendBuf[8];
	
	api_UART2_SendData(10,mMaininf.mUart2.SendBuf);
}
...
void api_Send_Signal(u8 mCmd,u8 OpenFlag)
{
	switch(mCmd)
	{
		case 0x01 :
			if(OpenFlag == 1)
			{
				mMaininf.mWork.mWorkSensorSendDropFlag = 1;
			}
			else
			{
